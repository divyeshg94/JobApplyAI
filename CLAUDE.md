...
Milestone 6 (tailored resume/cover-letter generation) is code-complete but not yet live-verified — see `docs/architecture.md` Status section for the full breakdown. Two migrations landed mid-session (deadline/classification columns, work-location-country) that may not be applied to the live DB yet — re-run `docs/schema-initial.sql` (idempotent) before testing. Milestones 7–8 haven't started.

## Next step

Verify milestone 6 live: one real "Generate documents" click on `/matches`, confirm the resume PDF doesn't fabricate work history (check bullets stay grounded in the original profile text), confirm cover letter reads sensibly, confirm "Mark applied" flips status. Then milestone 7: browser extension + autofill (`docs/architecture.md` §9 item 7).
